 ...
 *
 * The elements are held directly rather than behind a thunk: `dom.js` resolves
 * every one of them as it is imported, and it is imported above, so they are
 * all on hand by the time this line runs.
 */
const SLIDERS = [
...
function bindSlider({ input: slider, key, apply }) {
...
function resetSettings() {
  for (const { input: slider, key, apply } of SLIDERS) {
    removeRaw(key);
...
export function initSettings() {
  ...
  // The decimal separator and the unit are language-dependent, so all three
  // chips are rewritten — the sensitivity one included: it prints a decimal
  // too, and leaving it out left a French comma standing in the English build
  // until the slider was next touched.
  onLocaleChange(() => {
    applySensitivity(Number(el.sensitivity.value));
