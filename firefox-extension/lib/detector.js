 ...
 *
 * It runs in the engine window, which shows the stream it reads.
 *
 * The runtime and the models are the copies under `vendor/`, downloaded once
 * by `fetch-vendor.sh`: an extension page may only run scripts it ships
 * itself, so the CDN the web app loads from is not available here.
 */
...
export async function initDetector() {
  ...
  const fileset = await FilesetResolver.forVisionTasks(
    api.runtime.getURL(MEDIAPIPE.VISION_WASM)
  );

  faceLandmarker = await FaceLandmarker.createFromOptions(fileset, {
    baseOptions: {
      modelAssetPath: api.runtime.getURL(MEDIAPIPE.FACE_MODEL),
      delegate: "GPU"
    },
    runningMode: "VIDEO",
    numFaces: 1
  ...
  try {
    handLandmarker = await HandLandmarker.createFromOptions(fileset, {
      baseOptions: {
        modelAssetPath: api.runtime.getURL(MEDIAPIPE.HAND_MODEL),
        delegate: "GPU"
      },
      runningMode: "VIDEO",
      numHands: 2
