/**
 * The rule the whole extension exists for: the video plays while the mouth
 * stays closed.
 *
 ...
 * the video in the tabs.
 *
 * Both sinks are injected because the two audiences sit in different places:
 * `ui` is the engine window, which holds the camera, and `page` is every tab,
 * reached through the background script. Firefox will not open a camera from a
 * page with no visible window, which is why the rule runs in a window of its
 * own rather than in the background — see `engine/engine.js`.
 */
