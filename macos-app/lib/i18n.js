 ...
 *
 * The app ships in French only; strings live in `locales/fr.json` so that copy
 * can be edited without touching code. Adding a language back would mean
 ...
 * `t(key)`. The overlay the child sees holds no copy of its own: every
 * sentence it shows is translated in the engine and travels with the message
 * that asks for it.
 */
...
/** Both halves of a message, ready to hand to the overlay. */
 ...
 *
 * The pages are served over `app://` rather than `file://` precisely so that
 * this `fetch` — and the ES module graph, and the camera's secure context —
 * work the way they do in a browser. See `main/protocol.js`.
 */
export async function initI18n() {
  const response = await fetch(`/${I18N.PATH}/${I18N.LOCALE}.json`);
